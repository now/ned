...
private
  ...
  def print_tables(data, outfile = 'unichartables.h')
    row = []
...
EOF
  ...
  def print_decomp(data, outfile = 'unidecomp.h')
    row = []
    File.open(outfile, 'w') do |file|
      header_h = outfile.upcase.gsub(/[^A-Z0-9]/, '_')
      $defout = file
...
static const uchar cclass_data[][256] = {
EOH
...
EOL
  ...
  def print_composition_table(data, outfile = 'unicomp.h')
    first = Hash.new(0)
    second = Hash.new(0)
...
EOL
  ...
  def print_line_break(data, outfile = 'unibreak.h')
    row = []
    File.open(outfile, 'w') do |file|
      header_h = outfile.upcase.gsub(/[^A-Z0-9]/, '_')
      $defout = file
...
class Runner
  def main
    check_for_data_files('/home/pcp')
    data = CollectedData.new('/home/pcp', "\t")
